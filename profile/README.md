...
| **[RaBackend](https://github.com/Micla-DevTeam/NewRaBackend)** | The cloud backend & real-time services — API, media (MinIO), voice/video (LiveKit), and offline sync (PowerSync). | Bun · Fastify · TypeBox · PostgreSQL |
...
| **[RaMap](https://github.com/Micla-DevTeam/NewRaMap)** | Shared geospatial stack: vector tiles, routing, and geocoding for Tunisia, Morocco & Italy. | Martin · Valhalla · Photon |
...
```
  ...
  (NewRaApp)  ──▶  nginx ┼─ rtc.*     → LiveKit (voice/video)
   managed by            ├─ tiles.*   → Martin ┐
   RaMDM                 ├─ routing.* → Valhalla├─ NewRaMap
                         └─ geocode.* → Photon  ┘
        (NewRaGateway is the single entry point for all services)
```
